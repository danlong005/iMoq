...
dcl-proc argText;
  ...
  dcl-ds err likeds(imoq_err_t);
  if not imoq_getArg(upperName(obj) : proc : callNo : parmNo : value
                     : err);
    gMsg = %trimr(err.text);
    return *off;
  endif;
...
dcl-proc imoq_withChar export;
  ...
  if not addMatcher(h : parmNo : matcher : v);
    fail(gMsg);
  endif;
end-proc;
...
dcl-proc imoq_withNum export;
  ...
  if not addMatcher(h : parmNo : matcher : imoq_numText(value));
    fail(gMsg);
  endif;
end-proc;
...
dcl-proc imoq_withDate export;
  ...
  if not addMatcher(h : parmNo : matcher : %char(value : *iso));
    fail(gMsg);
  endif;
end-proc;
...
dcl-proc imoq_setParmNum export;
  ...
  if not addSet(h : parmNo : imoq_numText(value));
    fail(gMsg);
  endif;
end-proc;
...
dcl-proc imoq_setParmDate export;
  ...
  if not addSet(h : parmNo : %char(value : *iso));
    fail(gMsg);
  endif;
end-proc;
...
dcl-proc imoq_argNum export;
  ...
  dcl-s v varchar(1024);
  if not argText(obj : proc : callNo : parmNo : v);
    fail(gMsg);
    return 0;
  endif;
  monitor;
...
dcl-proc imoq_argPassed export;
  ...
  dcl-s v varchar(1024);
  dcl-s passed ind;
  if not argText(obj : proc : callNo : parmNo : v : passed);
    fail(gMsg);
    return *off;
  endif;
  return passed;
